(ns marketentry.marketentryllm
  ...
  the Namibia (NAM) public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "seeking-board-routing-determination?=" (:seeking-board-routing-determination? e)
                        " pe-category=" (:pe-category e)
                        " contract-type=" (:contract-type e)
                        " estimated-value=" (:estimated-value e)
                        " claimed-board-routing?=" (:claimed-board-routing? e)
