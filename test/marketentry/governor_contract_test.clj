  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Windhoek Fabrication (Pty) Ltd"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Windhoek Fabrication (Pty) Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest board-routing-mismatch-is-held-and-unoverridable
  (testing "declared :seeking-board-routing-determination? true but claimed routing doesn't match Annexure 1's own threshold matrix -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:board-routing-mismatch} (-> (store/ledger db) last :basis)))
