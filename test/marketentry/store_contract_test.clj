...
(defn- exercise [s]
  (store/commit-record! s {:effect :engagement/upsert
                           :value {:id "eng-x" :operator "X (Pty) Ltd" :jurisdiction "NAM"
                                   :base-fee 100 :monthly-rate 10 :monitoring-months 1
                                   :claimed-fee 110.0
                                   :seeking-board-routing-determination? true :pe-category :category-3
                                   :contract-type :works :estimated-value 20000000.0
                                   :claimed-board-routing? false
                                   :drafted? false :submitted? false :status :intake}})
  (store/commit-record! s {:effect :assessment/set
                           :path ["eng-x"]
                           :payload {:jurisdiction "NAM" :checklist ["a"] :spec-basis "x"}})
...
(deftest mem-and-datomic-parity
  (let [mem (store/seed-db)
    ...
    (is (= (:pe-category (:engagement m)) (:pe-category (:engagement d))))
