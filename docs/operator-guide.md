...
2. Register the client's intake: business type, target public function,
   prior filing history in Namibia if any.
3. Run the advisor in read-only mode against the CPBN e-procurement
   portal (eprocurement.gov.na).
4. Compare the checklist against the client's current documentation
   (BIPA registration status).
...
- named referral relationship with Namibian-licensed counsel or a
...
a working referral relationship with Namibian-licensed counsel or a
registered agent for whatever licensed representation the law of
Namibia requires for actual public-procurement filings.
